      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex flex-row-reverse items-center justify-between">
          <div className="flex flex-row-reverse items-center gap-4">
            <Link href="/" className="flex flex-row-reverse items-center gap-3 group transition-transform hover:scale-105">
          ...
          <nav className="flex flex-row-reverse items-center gap-2 md:gap-4 p-1.5 bg-dark-lighter/50 rounded-full border border-white/5 shadow-inner" dir="rtl">
            <HeaderLink href="/" icon={<IconBook2 className="w-5 h-5" />} label="المصحف الإلكتروني" />
            <HeaderLink href="/hadith" icon={<IconMessageCircle2 className="w-5 h-5" />} label="الأحاديث النبوية" />
          </nav>
        </div>
      </div>
    </header>
...
function HeaderLink({ href, icon, label }: { href: string, icon: React.ReactNode, label: string }) {
  return (
    <Link
      href={href}
      className="flex flex-row-reverse items-center gap-2 px-5 py-2.5 text-sm md:text-base font-bold text-gray-400 hover:text-white hover:bg-white/5 rounded-full transition-all duration-300 font-arabic"
    >
      {icon}
      <span className="hidden sm:inline-block mr-1">{label}</span>
    </Link>
  )
}