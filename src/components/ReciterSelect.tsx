...
export function ReciterSelect({ reciters, selectedReciter, onReciterChange }: ReciterSelectProps) {
    ...
    return (
        <Select.Root value={selectedReciter} onValueChange={onReciterChange}>
            <Select.Trigger
                className="flex items-center gap-3 px-6 py-3 rounded-2xl glass border-primary/20 hover:border-primary/40 focus:outline-none transition-all duration-300 group shadow-lg min-w-[220px] flex-row-reverse"
                dir="rtl"
            >
                    ...
                    <Select.Viewport className="p-2">
                        {reciters?.map((reciter) => (
                            <Select.Item
                                key={reciter.id}
                                value={reciter.id}
                                className="relative flex items-center px-6 py-3 text-sm text-gray-300 hover:text-white hover:bg-primary/20 rounded-xl cursor-pointer outline-none transition-colors data-[selected]:bg-primary data-[selected]:text-white mb-1 last:mb-0 flex-row-reverse text-right font-arabic"
                            >
                        ...
                        {(!reciters || reciters.length === 0) && (
                            <div className="px-6 py-3 text-sm text-gray-500 italic text-right font-arabic">جاري تحميل القراء...</div>
                        )}
                    </Select.Viewport>
                </Select.Content>
            </Select.Portal>
